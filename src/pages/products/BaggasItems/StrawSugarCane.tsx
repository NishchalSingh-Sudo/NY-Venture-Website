// src/pages/products/BiodegradableItems/StrawSugarCane.tsx
...
const StrawSugarCane: React.FC = () => {

  // Filter products directly from allDetailedProductsData
  // Using category and subCategory from your allProductsData.ts
  const sugarCaneStraws: DetailedProduct[] = allDetailedProductsData.filter(product =>
    product.category === 'biodegradable-items' && 
    product.subCategory === 'Biodegradable Straws' 
  ).filter(product => 
      product.name.includes('Sugar Cane Straws')
  ); 
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 font-inter">
        ...
        <div className="container mx-auto px-4 relative">
          <div className="max-w-4xl mx-auto text-center animate-fade-in">
            ...
            <p className="text-xl text-muted-foreground font-inter">
              Switch to sustainable sipping with our natural sugar cane straws, a fantastic biodegradable alternative to traditional plastic.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          {sugarCaneStraws.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {sugarCaneStraws.map((product) => (
                <ProductCard
                  key={product.id}
                  id={product.id}
                  name={product.name}
            ...
            <p className="text-center text-lg text-gray-700">No Sugar Cane Straws found at the moment. Please check back later!</p>
          )}
        </div>
      </section>
