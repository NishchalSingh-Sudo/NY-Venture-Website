// src/pages/products/BiodegradableItems/PlateSnacksTriangleSugarane.tsx
...
const PlateSnacksTriangleSugarane: React.FC = () => {

  // Filter products directly from allDetailedProductsData
  // Using category and subCategory from your allProductsData.ts
  const triangleSnackPlates: DetailedProduct[] = allDetailedProductsData.filter(product =>
    product.category === 'biodegradable-items' && 
    product.subCategory === 'Biodegradable Plates' 
  ).filter(product => 
      product.name.includes('Triangle Snack Plate') && 
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 font-inter">
        ...
        <div className="container mx-auto px-4 relative">
          <div className="max-w-4xl mx-auto text-center animate-fade-in">
            <h1 className="text-5xl font-bold text-foreground mb-6 font-playfair">
              Triangle Snack Plates - Sugar Cane
            </h1>
            <p className="text-xl text-muted-foreground font-inter">
              Discover our uniquely designed triangular snack plates, crafted from sustainable sugar cane, offering an eco-friendly and stylish option for your serving needs.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          {triangleSnackPlates.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {triangleSnackPlates.map((product) => (
                <ProductCard
                  key={product.id}
                  id={product.id}
                  name={product.name}
            ...
            <p className="text-center text-lg text-gray-700">No Triangle Snack Plates (Sugar Cane) found at the moment. Please check back later!</p>
          )}
        </div>
      </section>
...
export default PlateSnacksTriangleSugarane;