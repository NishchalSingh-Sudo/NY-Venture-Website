// src/pages/products/BiodegradableItems/WoodenFork.tsx
...
const WoodenFork: React.FC = () => {

  // Filter products directly from allDetailedProductsData
  // Using category and subCategory from your allProductsData.ts
  const woodenForks: DetailedProduct[] = allDetailedProductsData.filter(product =>
    product.category === 'biodegradable-items' && 
    product.subCategory === 'Biodegradable Cutlery' 
  ).filter(product => 
      product.name.includes('Wooden Forks')
  ); 
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 font-inter">
        ...
        <div className="container mx-auto px-4 relative">
          <div className="max-w-4xl mx-auto text-center animate-fade-in">
            ...
            <p className="text-xl text-muted-foreground font-inter">
              Choose our natural wooden forks for a truly sustainable dining experience, a sturdy and biodegradable alternative to plastic.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          {woodenForks.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {woodenForks.map((product) => (
                <ProductCard
                  key={product.id}
                  id={product.id}
                  name={product.name}
            ...
            <p className="text-center text-lg text-gray-700">No Wooden Forks found at the moment. Please check back later!</p>
          )}
        </div>
      </section>
